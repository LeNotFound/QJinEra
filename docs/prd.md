...
将 QQBot 从传统 Q&A 模型升级为**真正的赛博群友**，并蜕变为一个由时间、欲望与事件共同驱动的**自我进化型数字生命 (Self-Evolving Digital Being)**：  
- **有灵性**：不仅仅是回答问题，更能“吃瓜”、“捧哏”、感知情绪，甚至拥有独立的生理节律。  
- **有记忆**：像老朋友一样记住你的喜好、经历和状态，并随着时间推移加深了解。  
- **有“人味儿”**：拒绝秒回和长篇大论，模拟真人的打字延迟、分条发送和语气习惯。  
- **自由意志**：能够在后台自主编写插件 (Meta-Skill) 为用户创造惊喜。
...
## 4.3 记忆提取的实体消歧 (Entity Disambiguation / 待实现)
由于在 V4.0 中引入了 `subject_id` 概念以区分“信息是谁说的 (source_id)”和“信息是关于谁的 (subject_id)”，当对话中提及了第三方的群成员（如成员A提及成员B“璃落是洛尘的bot”），LLM 极其容易因为缺乏上下文名称-QQ映射而将 `subject_id` 错误地归属给当前发言者或 Bot 本身。
1. **获取群成员列表缓存**：维护一份当前群组的 `{群昵称: QQ号}` 字典。
2. **文本预检 (Pre-flight Regex)**：在将近期消息切片送入提取器（Extractor）之前，使用正则表达式扫描文本中是否提及了已知的群成员昵称。
3. **注入潜意识环境**：如果在文本中匹配到相关群成员的昵称，则在传给大模型提取器的 Prompt 尾部强行注入环境上下文补充（例如：`[环境补充资料：本群存在成员“璃落”(QQ:123456)和“洛尘”(QQ:654321)]`），使得 LLM 提取事实时能对得上号并将 `subject_id` 精准分配。

---

# 5. 🌟 **V4.0 核心模块 (Digital Being Engine)**

## 5.1 灵魂骨架与认知基建 (Infrastructure & Cognition)
*   **Jinja2 模板引擎**：废弃大段硬编码拼接，动态渲染当前的状态、情绪和上下文。
*   **UPEX 协议适配 (.ujson)**：将人设解耦，实现无缝切换多重人格。
*   **世界图鉴 (World Lore)**：在 SQLite 中建立全局共享的名词字典，防幻觉。
*   **心智理论 (ToM)**：在生成回复前加入思考链，分析用户的信念、意图和情绪。
*   **镜像人格**：通过对话反向生成用户的标准 .ujson 画像。

## 5.2 双驱动生命引擎 (The Dual-Driven Engine)
*   **交互驱动 (Energy Cost)**：根据对话深度动态计算 `energy_cost` 消耗。
*   **时间驱动 (Time-Driven)**：后台调度器控制基础代谢与离线模拟（看书发呆）。
*   **动态降级与省电模式**：精力跌破阈值时自动切入极度简短、慵懒的回复状态。

## 5.3 感官觉醒与外部连接 (Sensory & Implicit Search)
*   **彻底废弃传统 RAG 向量检索**，取而代之的是多模态与隐式调用。
*   **视觉模态优先**：拦截 `[CQ:image]`，结合多模态模型看懂群图片。
*   **表情包情绪解析**：将 `[CQ:face]` ID 翻译为带情绪色彩的文本标签辅助 ToM。
*   **隐式关怀 & MCP**：在感知气温下降隐式查天气，闲时利用 MCP 搜集新闻吃瓜。

## 5.4 自我进化与秘密工程 (Gödel Loop)
*   **元技能工具 (skill-creator)**：赋予她自主编写 Python 插件脚本的能力。
*   **自主热重载**：完成后台行为后，自主调用 API 接口完成补丁加载。
*   **沙盒容错**：代码执行崩溃时自动回滚和自我修复 Debug。

## 5.5 关系演化与灵魂绑定 (Soulbound)
*   **动态好感度与阶梯演化**：引入关系阶段，根据等级加载不同的 Jinja2 态度补丁。
*   **AT 立场锁定**：对非造物主锁死好感度，展现高情商疏离防御，对造物主展现绝对依赖。
